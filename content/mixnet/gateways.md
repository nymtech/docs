...
PEApps connect to gateways. Messages are automatically piped to connected clients and deleted from the gateway's disk storage. If a PEApp client is offline when a message arrives, it will be stored for later retrieval. When the client connects, all messages will be delivered, and deleted from the gateway's disk. At the moment (0.7.x release) messages are stored unencrypted on the gateway. In release 0.8.0 we will have end-to-end encryption. 

When it starts up, a PEApp's [client](../clients) registers itself with a gateway, and the gateway returns an access token. The access token plus the gateway's IP can then be used as a form of addressing for delivering packets.
